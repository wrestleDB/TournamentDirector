...
authentication = require './services/authenticationSessions'
...
app.post '/login', passport.authenticate 'local', {successRedirects: "/testing"}, (err, user, info) ->
    console.log "after auth: ", user
    # if err      then return next(err)
    # if not user then return next(null, false)#res.redirect('/login')

    # req.logIn(user, (err) ->
    #   console.log "made it past authenticate", req.isAuthenticated()
    #   console.log {err}
    #   if err then return next(err)
    #   next()
...
app.use (req, res, next) ->
  console.log "TESTING THIS HERE!!!", req.isAuthenticated()
...
#   console.error err
module.exports.sessionServer = app